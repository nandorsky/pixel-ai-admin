export default defineEventHandler(async (event) => {
  ...
  const url = `https://send.patent355.com/api/campaigns/${id}/leads${params.toString() ? `?${params.toString()}` : ''}`
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: error.message || 'Failed to fetch campaign leads from Email Bison'
    })
  }
})
