...
export default defineEventHandler(async (event) => {
  ...
  const template = body.htmlTemplate || buildSignupReminderTemplate()
  const subject = body.subject || 'Your Pixel credits are waiting'
  const results: { id: number; status: string; error?: string }[] = []

  for (const id of ids) {
    const { data: signup, error: fetchError } = await supabase
      .from('signups')
      .select('id, email, first_name, signup_reminder_sent_at')
      .eq('id', id)
      .single()
    ...
    if (signup.signup_reminder_sent_at) {
      results.push({ id, status: 'already_sent' })
      continue
    }

    const html = replaceSignupReminderPlaceholders(template, {
    ...
    try {
      ...
      if (updateError) {
        ...
        results.push({ id, status: 'error', error: 'Email sent but signup_reminder_sent_at not updated (RLS may be blocking updates)' })
      } else {
